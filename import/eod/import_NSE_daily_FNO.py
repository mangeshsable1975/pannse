...
future_expiry_list = list()
future_date_format = '%d%b%Y'
fromDate = date(2020,5,14)
toDate = date(2020,5,14)


#Clean up passed expiry data from table

def cleanupOldFnOData():
    session.query(NseFNODaily.expiry_date < todaysDate).delete()
    session.commit()
...
stocks = session.query(StockList).filter(StockList.downloaded == False,StockList.fno == True)

for stock in stocks:
    print("######################## Running for Stock:" + stock.stock_code)
    for expiry_date in expiry_dates:

        stock_ohlc_data = get_history(symbol=stock.stock_code, start=fromDate, end=toDate, expiry_date=expiry_date,
