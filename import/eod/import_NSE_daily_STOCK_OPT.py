...
async def getOptionData(stock_code):
	data = nse_urls.get_nifty_opt_chain_url(stock_code).text
	print(data)

trade_date = datetime.today()

stocks = session.query(StockList).filter(StockList.downloaded == False,StockList.fno == True,StockList.indices == False)
for stock in stocks:
...
print("Download Complete Option Data for all FNO Stocks")
	...
	#pe_values = [peData for peData in option_chain_json["filtered"]["data"]]
	#print(ce_values)
	#print(time.process_time() - start)
	#print(data)
	#print("######################## Running for Stock:" + stock.stock_code)
	#data = nse_urls.get_nifty_opt_chain_url(stock.stock_code).text
